    ...
    $.extend({
    	...
    	table: {
            ...
            responseHandler: function(res) {
                if (res.code == "0") {
                	var data = res.data;
                    return { rows: data.records, total:data.total };
                } else {
        ...
        treeTable: {
            ...
            init: function(options) {
                ...
                var treeTable = $('#bootstrap-table').bootstrapTreeTable({
                    ...
                    rootCodeValue : options.rootCodeValue | null //设置根节点code值
        	    });
    	...
    	modal: {
            ...
            //页面层
            openContent:function(title,content,width,height){
                layer.open({
                    type: 2,
                    fix: false,
                    //不固定
        ...
        operate: {
			...
			import:function(){
                ...
                $.modal.open("导入" + $.table._option.modalName, url,700,250);
			},
            // 添加信息
            add: function(id) {
            ...
            editFull: function(id) {
            	var url = $.table._option.updateUrl.replace("{id}", id);
            	$.modal.openFull("编辑" + $.table._option.modalName, url);
            },
            // 保存信息
			...
			//文件上传
			fileUpload:function(files,callback){
                var $files = $(files);
                // FormData，新的form表单封装，如下
    	...
    	common: {
            ...
            //加载行政区域
            loadArea:function(url,obj,params,defaultVal){
                var config = {
                    url: url,
                    type: "GET",
                    dataType: "json",
                    ...
                    success: function(data) {
                        ...
                        var html = '';
                        for(var e in data){
                            if(defaultVal === data[e].id){
                                html += '<option value='+data[e].id+' selected="selected">'+data[e].name+'</option>';
                            }else{
            ...
            //根据属性值查找对象
            findElem:function(array,attr,val){
            ...
            //操作按钮封装展示
            operationPanel:function(actions){
                var panel = "";
                if(!actions || actions.length <=0){
                    return panel;
                }
                panel +='<div class="btn-group">';
                panel +='	<button data-toggle="dropdown" class="btn btn-default btn-outline dropdown-toggle" aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog"></i> <span class="caret"></span></button>';
                panel +='	<ul class="dropdown-menu dropdown-menu-diy">';
